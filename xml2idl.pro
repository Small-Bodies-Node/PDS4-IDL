...
;	Written by Ed Shaya / U. of Maryland [April 27, 2012]
;	Modified by Ed Shaya [May 4, 2012] - 
; Changed the ANDs to &&s to speed it up and to fix a bug when using
;   IDL version 6 where index cannot be -1. (ES-7/5/15)
...
;---------------------------------------------
  ...
  IF (SIZE(paramArr,/type) NE 7) THEN $
   ...
   ; Remove quotes on values
   ; First remove double quotes if at beginning and end.
   IF (nodeValue NE '') THEN BEGIN
  	IF (STRMID(nodeValue,0,1) EQ '"' && $
	   STRMID(nodeValue,0,1,/reverse_offset) EQ '"') THEN $
   	   	nodeValue = STRMID(nodeValue,1,strlen(nodevalue)-2)
 ...
 ;    stop
	    ...
	    ; that is childvalue and child elements.
	    IF (childValue NE '') THEN BEGIN
		    ;;;if (childname eq '_text') then print,strmid(childvalue,1,2)
		   ;;; if strmid(childvalue,1,2) eq ')"' then stop
